...
# Exits with code 3 if PERPLEXITY_API_KEY is unset (caller can fall back to WebSearch).
set -euo pipefail
...
print(json.dumps({
  'model': sys.argv[1],
  'messages': [
    {'role': 'system', 'content': 'You are a precise financial research assistant. Cite every claim. Focus on VIX regime, volatility, and sector rotation context.'},
